...
class PadCollate:
    ...
    def __call__(self, batch):
        ...
        for _, cqt, target, note_attrib, _, full_target, _, _ in batch:
            ...
            # if frame_note_max_len < frame_level_note_attribs.shape[0]:
            #     frame_note_max_len = frame_level_note_attribs.shape[0]
...
class SoloDataset(Dataset):
    def __init__(self, track_name_list, data_dir):
        ...
        # ablation study
        #self.frame_level_note_attrib_dir = os.path.join(self.data_dir, "frame_level_pred")
...
class SoloDatasetModule(LightningDataModule):
    ...
    def prepare_data(self):
        ...
        #onset_dir = os.path.join(output_dir, "onset")
        ...
        #onset_time_sec_dir = os.path.join(output_dir, "onset_time_sec")
        ...
        if self.hparams.preprocess_on_training_start:
            ...
            if os.path.exists(output_dir):
                ...
                #os.makedirs(onset_time_sec_dir)
            else:
                os.makedirs(cqt_dir)
                #os.makedirs(onset_dir)
                os.makedirs(frame_level_note_attrib_dir)
